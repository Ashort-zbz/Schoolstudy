...
有两种达到这一目的的方法。第一种方法非常直观：只需在新类中产生现有类的对象。由于新的类是由现有类的对象所组成，所以这种方法称为组合。该方法只是复用了现有程序代码的功能，而非它的形式。第二种方法更细致一些，它按照现有类的类型来创建新类。无需改变现有类的形式，采用现有类的形式并在其中添加新代码。这种方式成为继承，而且编译器可以完成其中大部分工作。继承是面向对象程序设计的基石之一。
